<template>
  <div class="barang">
    <h1 class="text-center">This is a note page</h1>
    ...
    <table class="table table-striped mt-2">
      ...
      <tbody>
        <tr v-for="(item, index) in note" :key="item.id">
          ...
          <td>
            <router-link
              :to="{ name: 'admin-detailNote', params: { id: item.id } }"
            ...
            >
            <router-link
              :to="{ name: 'admin-updateNote', params: { id: item.id } }"
            ...
            >
            <router-link
              :to="{ name: 'admin-addBarangTumpukan', params: { id: item.id } }"
            ...
            >
            <router-link
              :to="{ name: 'admin-signature', params: { id: item.id } }"
...
export default {
  ...
  methods: {
    async fetchNote() {
      try {
        const response = await axios.get("http://localhost:8080/api/list-note/");
